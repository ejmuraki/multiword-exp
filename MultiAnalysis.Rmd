...
expressions_nofunction <- as.data.frame(removeWords(expression_coding$Expression, functions$Functions))

#Remove spaces left after function words removed
expressions_nofunction <- as.data.frame(str_squish(expressions_nofunction$`removeWords(expression_coding$Expression, functions$Functions)`))

#Rename column
expressions_nofunction <- rename(expressions_nofunction, "Expression_noFun" = "str_squish(expressions_nofunction$`removeWords(expression_coding$Expression, functions$Functions)`)")

#Bind columns to original
expressions_nofunction_all <- bind_cols(expression_coding, expressions_nofunction)
...
nofun_words <- expressions_nofunction_all %>% 
  separate(Expression_noFun, c("W1", "W2", "W3", "W4", "W5", "W6", "W7", "W8", "W9", "W10", "W11", "W12", "W13", "W14", "W15", "W16"), extra = "drop", remove = FALSE)

#Change to long format
nofun_words_long <- reshape2::melt(nofun_words, id.vars = c("Expression", "Code", "Expression_noFun"))
...
Code.labs <- c("Idiom/Fixed Expression", "Noun noun", "Participle verb", "All Expressions")
...
Code.labs <- c("Idiom/Fixed Expression", "Noun noun", "Participle verb")
names(Code.labs) <- c("Idiom", "Noun-noun", "Participle verb")

#Plot relationships
ggplot(conc_compare_plot, aes(Mean_C, Mean_Conc_Words)) + geom_point(size = 0.25, colour = "black", alpha = 0.15) + geom_smooth(method = lm, lwd = 1, colour = 4) + facet_grid(Ind_Words ~ Code, labeller = labeller(Ind_Words = Ind_Words.labs, Code = Code.labs)) + xlab ("Concreteness of Expression") + ylab("Concreteness of Individual Words") + theme(text = element_text(size = 20))
...
expressions_analysis_nofun %>% 
  dplyr::select(Expression, Code, Mean_BConc_F, Max_BConc_F, Min_BConc_F, Mean_C) %>% 
  drop_na() %>% 
  dplyr::group_by(Code) %>% 
  dplyr::summarize(Corr_mean = cor(Mean_C, Mean_BConc_F), Corr_max = cor(Mean_C, Max_BConc_F), Corr_min = cor(Mean_C, Min_BConc_F))
...
#Save file
ggsave(filename = file.path(figures_out, "Figure3.png"), width = 12, height = 12)
```
